...
def getMesures(generateur):
    ...
    for i, mesurevide in enumerate(generateur):
        mesures.append(Vec(mesurevide[1]*cos(mesurevide[0]), mesurevide[1]*sin(mesurevide[0]), i))
        if i > 400:
            break
    return mesures
...
def getCesures(mesures, mu_infini):
    ...
    for i in range(len(mesures)):
        ...
        if i < len(mesures)-2:
            ...
            rip=mesures[0].r
            ripp= mesures[1].r
            if (abs(ri- rim)<20 and abs(rim-rimm) < 20 and abs(ri-rip)>SEPARATION_MIN) \
                or ( abs(ri-rip)<20 and abs ( rip- ripp) < 20 and abs(ri-rim)> SEPARATION_MIN) \
                or ( abs(ri-rip)+abs(ri-rim ) >2* SEPARATION_MIN): # and abs(rim-rimm)+abs(rip-ripp)<50: cesures.append (Vec(mesures[i].x,mesures[i].y))
                if not newItem:
                    newItem = True
                    itemCount = 0
                ...
                if itemCount == 2 and ri>mu_infini*0.98:
                    newItem=False # TODO:récupérer le centre des obstacles, leur largeur
...
if __name__ == '__main__':
    ...
    for i in range(50):
        ...
        axes. axvline(0, 0,ls= "--" ,linewidth=0.35,color="g") # Les obstacles aléatoires
        nb_obst = randint(3,6)
